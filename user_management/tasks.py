...
#Tache Celery pour envoi des rapports d'import aux admins
@shared_task
def send_import_report_email(recipient_email, report_summary):
    """
    Envoie un email de rapport d'importation aux administrateurs
    :param recipient_email: adresse email du destinataire
    :param report_summary: texte ou html du rapport
    """
    try:
        subject = "Rapport d'import utilisateurs BCEF"
        message = f"Bonjour,\n\nVoici le rapport de la dernière importation d'utilisateurs :\n\n{report_summary}\n\nCordialement,\nÉquipe BCEF"
        send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, [recipient_email])
        logger.info(f"Import report email sent to {recipient_email}")
    except Exception as e:
        logger.error(f"Failed to send import report email to {recipient_email}: {e}")
        raise
...
def clean_expired_tokens():
    from user_management.models import User  # Assuming you have a User model
    now = timezone.now()
